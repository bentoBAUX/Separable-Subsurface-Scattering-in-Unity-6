Shader "bentoBAUX/Master Shaders/PBR Master"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Assets/Shaders/Library/Maths/ParallaxMapping.hlsl"


            half3 ProcessNormals(Varyings IN, float2 uv)
            {
                // Tangent-space normal from normal map
                half3 nTS = UnpackNormal(SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, uv));
                nTS.xy *= _NormalStrength;
                nTS = normalize(nTS);

                half3 mnTS = UnpackNormal(SAMPLE_TEXTURE2D(_MicroNormalMap, sampler_MicroNormalMap, uv * _MicroNormalScale));
            ...
            float3 LightLoop(Surf surfaceData, InputData inputData)
            {
                ...
                lit += float4(ambient,1);
                #elif defined(_LM_PBR)
                float3 ambient = 0.1 * c * surfaceData.ao * SampleSH(surfaceData.normalWS);
                lit += float4(ambient,1);
                #endif
            ...
            float4 frag(Varyings IN) : SV_Target0
            {
                ...
                float2 uv = SteepParallaxMapping(_DepthMap, IN.uv, normalize(mul(IN.TBN, inputData.viewDirectionWS)), _NumberOfLayers, _DepthScale);
                if (uv.x > 1.0 || uv.y > 1.0 || uv.x < 0.0 || uv.y < 0.0) discard;
                inputData.normalWS = ProcessNormals(IN, uv);
                ...
                surfaceData.baseColor = _BaseColor * SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, uv);
                surfaceData.alpha = surfaceData.baseColor.a * _BaseColor.a;
                surfaceData.uv = uv;

                #ifdef _USE_ROUGHNESS_MAP
                surfaceData.roughness = SAMPLE_TEXTURE2D(_RoughnessMap, sampler_RoughnessMap, uv).r * _RoughnessStrength;
                #else
                surfaceData.roughness = _Roughness;
                #endif

                #ifdef _USE_METALLIC_MAP
                surfaceData.metallic = SAMPLE_TEXTURE2D(_MetallicMap, sampler_MetallicMap, uv).r * _MetallicStrength;
                #else
                surfaceData.metallic = _Metallic;
                #endif

                #ifdef _USE_SPECULAR_MAP
                surfaceData.specular = SAMPLE_TEXTURE2D(_SpecularMap, sampler_SpecularMap, uv).r * _SpecularStrength;
                #else
                surfaceData.specular = _Specular;
                #endif

                surfaceData.ao = SAMPLE_TEXTURE2D(_AOMap, sampler_AOMap, uv).r * _AOStrength;
                surfaceData.thickness = SAMPLE_TEXTURE2D(_ThicknessMap, sampler_ThicknessMap, uv).r * _ThicknessMultiplier;

                // Calculate Lighting
                float3 lighting = LightLoop(surfaceData, inputData);

                float4 finalColour = float4(lighting.rgb, surfaceData.alpha);
                ApplyFog(finalColour, IN.positionWS);
